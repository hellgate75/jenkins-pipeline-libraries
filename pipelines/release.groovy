...
try {
    ...
    node() {
        ...
        stage('create package') {
            ...
            withCredentials([usernamePassword(credentialsId: 'github-jenkins-jenkinsfy', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
        ...
        stage('promote package') {
            bintray.upload('bintray-credentials', pomLib.artifactId(pwd() + "/pom.xml"), pomLib.version(pwd() + "/pom.xml"), 'zip', 'target/*.zip', 'jenkinsfy', 'maven')
        }
